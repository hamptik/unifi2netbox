...
def apply_extra_device_tag(nb, nb_device, tag_name):
    """
    Attach `tag_name` to `nb_device` if it is set and not already present.

    Used to honor the optional SYNC_DEVICE_TAG env var (e.g. for monitoring
    integration). Empty / whitespace-only tag_name is a no-op — keeps the sync
    free of any deployment-specific tag pollution by default.
    """
    if not tag_name or not tag_name.strip():
        return
    tag_name = tag_name.strip()
    tag = ensure_tag(nb, tag_name)
    if not tag:
        return
    current_tag_ids = [t.id for t in (nb_device.tags or [])]
    if tag.id in current_tag_ids:
        return
    current_tag_ids.append(tag.id)
    nb_device.tags = current_tag_ids
    nb_device.save()
    logger.info(f"Added '{tag_name}' tag to device {nb_device.name}.")
...
def process_device(unifi, nb, site, device, nb_ubiquiti, tenant, unifi_device_ips=None, unifi_site_obj=None):
    ...
    try:
        ...
        if nb_device:
            # Optional device tag (e.g. for monitoring integration). Driven by
            # SYNC_DEVICE_TAG env var. Empty by default → no tag is added.
            apply_extra_device_tag(nb, nb_device, (os.getenv("SYNC_DEVICE_TAG") or "").strip())
...
if __name__ == "__main__":
    ...
    if not nb_ubiquiti:
        nb_ubiquiti = nb.dcim.manufacturers.create({'name': 'Ubiquiti Networks', 'slug': 'ubiquiti'})
        if nb_ubiquiti:
